...
// --- createProposal function is unchanged ---
exports.createProposal = async (req, res) => {
    // ... (existing code is unchanged)
    try {
        const freelancerId = req.user._id;
        const { projectId } = req.body;
        if (req.user.role !== 'freelancer') { return res.status(403).json({ success: false, message: 'Only freelancers can submit proposals.' }); }
        const project = await Project.findById(projectId);
        if (!project || project.status !== 'open') { return res.status(404).json({ success: false, message: 'Project not found or not open for bidding.' }); }
        const proposal = await Proposal.create({ ...req.body, freelancerId, status: 'submitted' });
        if (!project.proposals.includes(proposal._id)) { project.proposals.push(proposal._id); await project.save(); }
        const clientMessage = `New bid received on your project: "${project.title}". Bid Amount: ${req.body.bid.amount} ${req.body.bid.currency}.`;
        await Notification.create({ recipientId: project.clientId, proposalId: proposal._id, projectId: projectId, message: clientMessage, type: 'new_bid' });
...
// --- getProposalsByProject function is unchanged ---
exports.getProposalsByProject = async (req, res) => {
    // ... (existing code is unchanged)
    try {
        const { projectId } = req.params;
        const project = await Project.findById(projectId);
        if (!project || project.clientId.toString() !== req.user._id.toString()) { return res.status(403).json({ success: false, message: 'Not authorized to view proposals for this project.' }); }
...
// ✅ ADDED: New function to approve a proposal directly
