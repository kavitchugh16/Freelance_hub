...
// Helper function to find a wallet or create one if it doesn't exist
const getOrCreateWallet = async (userId) => {
    let wallet = await Wallet.findOne({ user: userId }); //
    if (!wallet) {
        wallet = await Wallet.create({ user: userId, balance: 0, transactions: [] }); //
...
exports.releaseMilestonePayment = async (clientId, freelancerId, amount, milestoneDescription) => {
    ...
    session.startTransaction();
    try {
        // Find client wallet and ensure funds
        const clientWallet = await Wallet.findOne({ user: clientId }).session(session);
        if (!clientWallet || clientWallet.balance < amount) {
            await session.abortTransaction();
            session.endSession();
        ...
        // Find or create freelancer wallet within the same session
        let freelancerWallet = await Wallet.findOne({ user: freelancerId }).session(session);
        if (!freelancerWallet) {
            freelancerWallet = (await Wallet.create([{ user: freelancerId, balance: 0 }], { session }))[0];
        }
        ...
        // 3. Save both wallets within the transaction
        await clientWallet.save({ session });
        ...
        // If any error occurred, abort the transaction to roll back changes
        await session.abortTransaction(); //
        console.error('Milestone payment transaction failed:', error.message);
        throw error;
    } finally {
