...
import React, { useEffect, useState } from "react";
import axios from "axios";
import { FaBell, FaCheckCircle, FaProjectDiagram, FaArrowDown } from "react-icons/fa"; // Added FaArrowDown
...
interface Notification {
    ...
    message: string;
    isRead: boolean;
    type: 'new_bid' | 'bid_accepted' | 'outbid'; // Added 'outbid' type
    projectId: { _id: string, title: string } | null;
    createdAt: string;
}
...
const FreelancerNotifications: React.FC = () => {
    ...
    // --- ✅ ADDED: Function to render different icons based on notification type ---
    const renderIcon = (type: Notification['type']) => {
        switch (type) {
            case 'bid_accepted':
                return <FaCheckCircle className="text-green-500 flex-shrink-0 mt-1" size={24} />;
            case 'outbid':
                return <FaArrowDown className="text-orange-500 flex-shrink-0 mt-1" size={24} />;
            default:
                return <FaProjectDiagram className="text-indigo-500 flex-shrink-0 mt-1" size={24} />;
        }
    };
