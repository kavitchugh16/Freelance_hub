...
import React from 'react';
import { useNavigate } from 'react-router-dom';
import { FaUser, FaWallet, FaProjectDiagram, FaBell, FaBriefcase } from 'react-icons/fa';

const FreelancerDashboard = () => {
  const navigate = useNavigate();

  return (
    // ✅ CHANGED: Adjusted grid to fit 5 columns on large screens
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
      ...
      >
        <FaBriefcase size={40} className="mb-2 text-blue-600" />
        <h2 className="text-lg font-bold">My Projects</h2>
      </div>

      {/* ✅ RESTORED: The Notifications card is now back */}
      <div
        className="bg-pink-100 p-6 rounded-lg shadow-md flex flex-col items-center cursor-pointer hover:bg-pink-200"
        onClick={() => navigate('/freelancer/notifications')}
      >
