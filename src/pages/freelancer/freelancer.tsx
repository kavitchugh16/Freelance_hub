...
import axios from "axios";
import { useAuth } from "../../contexts/AuthContext";
import { FaMoneyBillWave, FaMinusCircle, FaHistory, FaArrowCircleDown } from "react-icons/fa";

interface Transaction {
    _id: string; amount: number; type: 'withdrawal' | 'milestone_payment_release';
...
const getTransactionInfo = (tx: Transaction) => {
    switch (tx.type) {
        case 'withdrawal':
            return { icon: FaMinusCircle, color: 'text-red-600', bgColor: 'bg-red-50', sign: '-' };
        case 'milestone_payment_release':
            return { icon: FaArrowCircleDown, color: 'text-green-600', bgColor: 'bg-green-50', sign: '+' };
        default:
...
const FreelancerWallet: React.FC = () => {
    ...
    useEffect(() => { if (user?.role === 'freelancer') fetchWalletDetails(); }, [user]);
    const handleWithdraw = async (e: React.FormEvent) => { /* ... existing withdraw logic ... */ };
